...
!> \ingroup Solvers
!-----------------------------------------------------------------------------
SUBROUTINE inverseIceFluxUncertaintySolver( Model,Solver,dt,TransientSimulation )
  USE DefUtils
  USE Differentials
  USE MaterialModels
  IMPLICIT NONE
  ...
  LOGICAL ::&
       ...
       UseBodyForce = .TRUE., ApplyDirichlet=.FALSE., ApplyMinMax=.False.,  ALEFormulation=.FALSE. , &
  ...
  INTEGER, POINTER ::&
       ThickPerm(:), DHDTPrem(:),FlowPerm(:), NodeIndexes(:),EdgeMap(:,:),SlopePerm(:),mbSlopePerm(:)


  REAL(KIND=dp) :: &
       at,st,totat,totst,Norm,PrevNorm,LocalBottom, cv, &
       Relax, MaxDisp, maxdh,LinearTol,NonlinearTol,RelativeChange,&
       smallestpossiblenumber, rr, ss, slope_threshold

  REAL(KIND=dp), POINTER :: ForceVector(:), Thick(:),DHDT(:),PreH(:,:), &
       FlowSolution(:),  PointerToResidualVector(:), SlopeSolution(:), mbSlopeSolution(:)

  REAL(KIND=dp), ALLOCATABLE :: ResidualVector(:), &
       STIFF(:,:),SourceFunc(:),FORCE(:), TimeForce(:), LOAD(:),&
       MASS(:,:), Velo(:,:), mbSlope(:,:), Flux(:,:), LowerLimit(:), UpperLimit(:), &
       OldValues(:), OldRHS(:),StiffVector(:),MeshVelocity(:,:)

  CHARACTER(LEN=MAX_NAME_LEN)  :: SolverName, VariableName, EquationName, FlowSolName, StabilizeFlag, SlopeSolName, mbSlopeSolName

  TYPE(Nodes_t)   :: ElementNodes
  TYPE(Element_t),POINTER :: CurrentElement
  TYPE(Variable_t), POINTER :: FlowSol, VarThickResidual,DHDTSol, SlopeSol, mbSlopeSol
...
!
  ...
  SAVE STIFF, MASS, FORCE, &
       ...
       ResidualVector, StiffVector, MeshVelocity, mbSlope
  ...
  SolverName = 'IceFlowSolver ('// TRIM(Solver % Variable % Name) // ')'
  ...
  ELSE
     ...
     END IF
  END IF
  ApplyMinMax = GetLogical( SolverParams, &
       'Apply MinMax', Found)
  IF ( .NOT.Found ) ApplyMinMax = .False.
  ...
  !------------------------------------------------------------------------------
   ...
   mbSlopeSolName =  GetString(SolverParams ,'MB Slope Variable Name', Found)
   IF(.NOT.Found) THEN        
        WRITE(Message,'(A)') &
   ...
   ELSE
        ...
        ELSE
             ...
             CALL Fatal(SolverName,Message)           
        END IF
        mbSlopeSol => VariableGet( Solver % Mesh % Variables, mbSlopeSolName )
        IF ( ASSOCIATED( mbSlopeSol ) ) THEN
             mbSlopePerm     => mbSlopeSol % Perm
             mbSlopeSolution => mbSlopeSol % Values
        ELSE
             WRITE(Message,'(A,A,A)') &
                    'No variable >',mbSlopeSolName,'< found'
             CALL Fatal(SolverName,Message)
        END IF
   END IF
          ...
          DO i=1,n
             j = NSDOFs*SlopePerm(NodeIndexes(i))
              ...
              IF((DIM == 2) .AND. (NSDOFs == 1)) THEN
                 Velo(1,i) = (-1.0)*(-1.0)*SlopeSolution( j )
                 Velo(2,i) = (-1.0)*0.0_dp
                 mbSlope(1,i) = (-1.0)*(-1.0)*mbSlopeSolution( j )
                 mbSlope(2,i) = (-1.0)*0.0_dp
              !2D problem - 2D Thickness evolution (plane view pb)
              ELSE IF ((DIM == 2) .AND. (NSDOFs == 2)) THEN
                 Velo(1,i) = (-1.0)*(-1.0)*SlopeSolution( j-1 )
                 Velo(2,i) = (-1.0)*(-1.0)*SlopeSolution( j )
                 mbSlope(1,i) = (-1.0)*(-1.0)*mbSlopeSolution( j-1 )
                 mbSlope(2,i) = (-1.0)*(-1.0)*mbSlopeSolution( j )
...
!              ELSE
       ...
       ELSE
        ...
        IF (ASSOCIATED( BodyForce ) ) THEN
              LOAD(1:n) = LOAD(1:n) +   &
                      GetReal( BodyForce, 'delta Top Surface Accumulation', Found )
              LOAD(1:n) = LOAD(1:n) +   &
                      abs(GetReal( BodyForce, 'grad_FLUXdeltaSPEED', Found ))
        END IF
        ...
        CALL LocalMatrix( STIFF, MASS, FORCE,&
             ...
             TransientSimulation,&
             ALEFormulation,slope_threshold, mbSlope)
     ...
     IF (ApplyDirichlet.AND.(.NOT.ApplyMinMax)) THEN
        ...
        IF (ApplyDirichlet) THEN
           ...
           DO i=1,Model % NumberOfNodes
              ...
              IF ((LimitedSolution(i,1)).AND.(Thick(l)-LowerLimit(i)<0.0_dp )) THEN
                 If (ApplyMinMax) Thick(l)=LowerLimit(i)
                 ActiveNode(i,1) = .TRUE.
              END IF
              IF ((LimitedSolution(i,2)).AND.(Thick(l)-UpperLimit(i)>0.0_dp )) THEN
                 If (ApplyMinMax) Thick(l)=UpperLimit(i)
                 ActiveNode(i,2) = .TRUE.
              END IF
   ...
   CONTAINS
     ...
     SUBROUTINE LocalMatrix( STIFF, MASS, FORCE,&
       ...
       REAL(KIND=dp) ::&
            ...
            norm,slope_threshold,counter,normit
       ...
       DO t = 1,IntegStuff % n
          ...
          IF(norm.lt.TAN(slope_threshold/180*3.14159))THEN
             normit=TAN(slope_threshold/180*3.14159)
          ELSE
             normit = norm
          ENDIF
          ! THIS LOWER LIMIT IS IMPORTANT
          ! J.J. Fuerst
          ! it should introduce a bias in the flux solution BUT I did
          ! not observe it for Austfonna and Vestfonna. However, if
          ! the condition is not included the flux becomes very high 
          ! near the divide.
          ! I tried to renormalise but could not improve the solution

          IF (norm.gt.0.0) THEN
            DO i=1,NSDOFs
              Vgauss(i) = Vgauss(i)/normit
            ENDDO
          ! safety case if surface topography completely flat
          ! this can happen for singular points near the divide
          ! (on Spitsbergen I did not see this) but also in regions
          ! with a bogus DEM (Southern Spitsbergen; I fear that
          ! surface elevations were pushed outwards for some glaciers)
          ELSE ! safety case if surface topography completely flat
            normit = 0.0_dp
            DO i=1,NSDOFs
              Vgauss(i) = SUM( Basis(1:nCoord)*(mbSlope(i,1:nCoord)))
              normit = normit + Vgauss(i)**2.0
            ENDDO
            if (normit.gt.0.0) then
              DO i=1,NSDOFs
              Vgauss(i) = Vgauss(i)/normit**0.5
              ENDDO
            else
              DO i=1,NSDOFs
              Vgauss(i) = 1.0/2.0**0.5
              ENDDO
            endif
          ENDIF
     ...
     !------------------------------------------------------------------------------
   END SUBROUTINE inverseIceFluxUncertaintySolver
!------------------------------------------------------------------------------
