...
!> \ingroup Solvers
!-----------------------------------------------------------------------------
SUBROUTINE IceFluxUncertaintySolver( Model,Solver,dt,TransientSimulation )
  USE DefUtils
  USE Differentials
  USE MaterialModels
  IMPLICIT NONE
          ...
          DO i=1,n
             j = NSDOFs*SlopePerm(NodeIndexes(i))
              ...
              IF((DIM == 2) .AND. (NSDOFs == 1)) THEN
                 ...
                 Velo(2,i) = 0.0_dp
                 mbSlope(1,i) = (+1.0)*mbSlopeSolution( j )
                 mbSlope(2,i) = 0.0_dp
              ...
              ELSE IF ((DIM == 2) .AND. (NSDOFs == 2)) THEN
                 ...
                 mbSlope(1,i) = (+1.0)*mbSlopeSolution( j-1 )
                 mbSlope(2,i) = (+1.0)*mbSlopeSolution( j )
...
!              ELSE
   ...
   CONTAINS
     ...
     !------------------------------------------------------------------------------
   END SUBROUTINE IceFluxUncertaintySolver
!------------------------------------------------------------------------------
