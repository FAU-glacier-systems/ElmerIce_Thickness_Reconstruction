...
! * 
! *****************************************************************************
SUBROUTINE NodalSlopeSolver( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
!******************************************************************************
!
!  ARGUMENTS:
...
!------------------------------------------------------------------------------
  ...
  TYPE(Variable_t), POINTER :: PointerToVariable, ZsSol
  TYPE(GaussIntegrationPoints_t) :: IP

  LOGICAL,SAVE :: AllocationsDone = .FALSE.
  LOGICAL,SAVE :: FEConsistent=.False.
  LOGICAL :: Found
  LOGICAL :: stat
  ...
  REAL(KIND=dp), POINTER :: VariableValues(:), Zs(:)
  REAL(KIND=dp) :: U,V,W
  REAL(KIND=dp) :: dsdx(2)
  ...
  CHARACTER(LEN=MAX_NAME_LEN),SAVE :: SolverName,VarName
...
!------------------------------------------------------------------------------
  ...
  WRITE(SolverName, '(A)') 'NodalSlopeSolver'

!------------------------------------------------------------------------------
!    Get variables needed for solution
!------------------------------------------------------------------------------
        VarName =  GetString( Solver % Values,'Variable Name', Found)
        If (.NOT.Found) VarName='Zs'
        ZsSol => VariableGet( Solver % Mesh % Variables, trim(VarName) )
        IF (ASSOCIATED(ZsSol)) THEN
           Zs => ZsSol % Values
           ZsPerm => ZsSol % Perm
        ELSE
           WRITE(Message, '(A,A)') 'Could not find variable :',trim(VarName)
   ...
   !!!!!! Compute surface slope element wise
             ...
             Do i=1,n
                  ...
                  IF (FEConsistent) THEN
                     ...
                     DO j=1,IP % n
                        ...
                        dsdx(1)=SUM( Zs(ZsPerm(NodeIndexes(1:n))) * dBasisdx(1:n,1) )
                        IF (STDOFs == 2) &
                           dsdx(2)=SUM( Zs(ZsPerm(NodeIndexes(1:n))) * dBasisdx(1:n,2) )
                  ...
                  ELSE
                     ...
                     grad(STDOFs*(k-1)+1) = grad(STDOFs*(k-1)+1) + &
                             SUM( Zs(ZsPerm(NodeIndexes(1:n))) * dBasisdx(1:n,1) )

                     IF (STDOFs == 2) grad(STDOFs*(k-1)+2) =grad(STDOFs*(k-1)+2) + &
                             SUM( Zs(ZsPerm(NodeIndexes(1:n))) * dBasisdx(1:n,2) )
                 END IF
       ...
       END IF !end do parallel reduction
        ...
        Do t=1,Solver % Mesh % NumberOfNodes
             ...
             VariableValues(STDOFs*(Permutation(t)-1)+1) = grad(STDOFs*(t-1)+1) / weight(t)
             IF (STDOFs == 2) VariableValues(STDOFs*(Permutation(t)-1)+2)=grad(STDOFs*(t-1)+2) / weight(t)
          END DO
...
!------------------------------------------------------------------------------
END SUBROUTINE NodalSlopeSolver
!------------------------------------------------------------------------------
