...
! Create a permutation table from NodePerm
           ...
           NPoints=ind
           allocate(xx(VarDOFs*Npoints),gg(VarDOFs*Npoints))
...
!jjf     print *,'Jean Krug optimises',VarDOFs
    ...
    ! and receive results from partion 0
     ...
     if (Solver %  Matrix % ParMatrix % ParEnv % MyPE .ne.0) then
                     ...
                     call MPI_SEND(x(1),VarDOFs*NActiveNodes,MPI_DOUBLE_PRECISION,0,8003,MPI_COMM_WORLD,ierr)
                     call MPI_SEND(g(1),VarDOFs*NActiveNodes,MPI_DOUBLE_PRECISION,0,8004,MPI_COMM_WORLD,ierr)
                     call MPI_RECV(x(1),VarDOFs*NActiveNodes,MPI_DOUBLE_PRECISION,0,8005,MPI_COMM_WORLD,status, ierr )
                     ...
                     Do i=2,Npes
                       call MPI_RECV(xtot(ni),VarDOFs*NodePerPe(i),MPI_DOUBLE_PRECISION,i-1,8003,MPI_COMM_WORLD, status, ierr )
                       call MPI_RECV(gtot(ni),VarDOFs*NodePerPe(i),MPI_DOUBLE_PRECISION,i-1,8004,MPI_COMM_WORLD, status, ierr )
            ...
            call m1qn3 (simul_rc,Euclid,ctonbe,ctcabe,VarDOFs*NPoints,xx,f,gg,dxmin,df1, &
            ...
            ni=1+VarDOFs*NActiveNodes
            Do i=2,Npes
                  call MPI_SEND(xtot(ni),VarDOFs*NodePerPe(i),MPI_DOUBLE_PRECISION,i-1,8005,MPI_COMM_WORLD,ierr)
