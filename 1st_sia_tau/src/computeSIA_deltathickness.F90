...
!
! *****************************************************************************
SUBROUTINE computeSIA_deltathickness( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
!******************************************************************************
!
!
...
!
  ...
  CHARACTER(LEN=MAX_NAME_LEN) :: VarSolName,FluxVarSolName,deltaFluxVarSolName,SlopeVarSolName,ViscVarSolName
  TYPE(Variable_t), POINTER :: Variable,FluxVariable,deltaFluxVariable,SlopeVariable,Hsia,ViscVariable
  REAL(KIND=dp), POINTER :: Values(:),FluxValues(:),deltaFluxValues(:),SlopeValues(:),ViscValues(:)
  INTEGER, POINTER :: Perm(:),FluxPerm(:),deltaFluxPerm(:),SlopePerm(:),ViscPerm(:)
!
  INTEGER :: ok
  INTEGER,SAVE :: nobs
...
!
  real(kind=dp) :: DSDX,DSDY,FLUX,THI,SLOPE,RF,VISC,HOBS,dFLUX
  real(kind=dp) :: MFLUX,kappa,MFLUX_S
  real(kind=dp) :: FLUX_MIN_S,FLUX_MAX_S,FLUX_MIN,FLUX_MAX
...
!
  ...
  save SolverName,VarSolName,FluxVarSolName,deltaFluxVarSolName,SlopeVarSolName,ViscVarSolName
...
!! Name of the variable to regularise
  deltaFluxVarSolName =  GetString( SolverParams,'delta Flux Variable Name', Found)
      IF(.NOT.Found) THEN
              CALL WARN(SolverName,'Keyword >delta Flux Variable Name< not found in section >Solver<')
      END IF
...
!-------------------------------------------------------------------------------
    ...
    deltaFluxVariable => VariableGet( Solver % Mesh % Variables, deltaFluxVarSolName  )
    IF ( ASSOCIATED( deltaFluxVariable ) ) THEN
            deltaFluxValues => deltaFluxVariable % Values
            deltaFluxPerm => deltaFluxVariable % Perm
    ELSE
            WRITE(Message,'(A,A,A)') &
                   'No variable >',deltaFluxVarSolName,' < found'
            CALL FATAL(SolverName,Message)
    END IF
...
END SUBROUTINE computeSIA_deltathickness
