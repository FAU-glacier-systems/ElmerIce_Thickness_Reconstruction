...
!
! *****************************************************************************
SUBROUTINE POI_output_thickness( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
!******************************************************************************
  USE ParticleUtils
  USE GeneralUtils
...
!  CHARACTER(LEN=MAX_NAME_LEN) :: CostSolName
  CHARACTER(LEN=MAX_NAME_LEN) :: ThickName,FluxName,SlopeName,SurfaceName
  CHARACTER(LEN=MAX_NAME_LEN) :: ObsFileName,ResFileName,Fname
  TYPE(Element_t),POINTER ::  Element
  TYPE(Variable_t), POINTER :: TimeVar
!  TYPE(Variable_t), POINTER :: CostVar
  TYPE(Variable_t), POINTER :: ThickVariable,FluxVariable,SlopeVariable,SurfaceVariable
  TYPE(ValueList_t), POINTER :: SolverParams,Material
  TYPE(Nodes_t) :: ElementNodes
  REAL(KIND=dp), POINTER :: ThickValues(:),FluxValues(:),SlopeValues(:),SurfaceValues(:)
  INTEGER, POINTER :: NodeIndexes(:),NodeIndices(:)
  INTEGER, POINTER :: ThickPerm(:),FluxPerm(:),SlopePerm(:),SurfacePerm(:)
...
!,VariableName,ViscVariableName,FricVariableName
  save ThickName,FluxName,SlopeName,SurfaceName
  save ResFileName
!  save ElementNodes
...
!!!!!!! Check for parallel run 
    ...
    END IF
     ...
     WRITE(SolverName, '(A)') 'POI_output_thickness'
...
!   V    = 0.0_dp
   ...
   do i=1,nobs
     ...
     if (i.eq.1)then
       ...
       yy0=xobs(i,2)
       xx(i)=xobs(i,1)
       !yy(i)=nint((xobs(i,2))*1000.)/1000.
       yy(i)=xobs(i,2)
       !nn=nn+1
       !print *,'FIRST ROUND : START'
       !print *,nn,mm,xobs(i,1),xx(i)
       !print *,nn,mm,xobs(i,2),yy(i)
       !print *,'FIRST ROUND : END'
     elseif(i.eq.2)then
       dxx=float(nint((xobs(i,1)-xx0)*10.))/10.
       dyy=float(nint(0.0))
       nn=nint((xobs(i,1)-xx0)/dxx)
       mm=0
       xx(i)=xx0+nn*dxx
       !yy(i)=nint((yy0+mm*dyy)*1000.)/1000.
       yy(i)=yy0+nn*dyy
       ...
       if(abs(xobs(i,2)-yy(i-1)).gt.3*abs(dyy))then
         if(mm.eq.0)then
           dyy=float(nint((xobs(i,2)-yy0)*10.))/10.
         endif
         !nn=0
         !mm=mm+1
       endif
       nn=nint((xobs(i,1)-xx0)/dxx)
       if(abs(dyy).gt.0)then
         mm=nint((xobs(i,2)-yy0)/dyy)
       endif
       xx(i)=xx0+nn*dxx
       yy(i)=yy0+mm*dyy
       !nn=nn+1
...
!          WRITE(IO,'(501(F16.8,3x))') xx(jj),yy(jj),-9999.
...
!            END IF
          IF(.NOT.Parallel)THEN
          IF(THI_S(jj).GT.0.0)THEN
          IF(FLUX.lt.100.and.SLOPE.lt.1e-2.and.SURF.gt.100)then
          WRITE(IO,'(3(F16.8,1x))') xx(jj),yy(jj),-9999.
          ELSE
          WRITE(IO,'(3(F16.8,1x))') xx(jj),yy(jj),THI_S(jj)
          ENDIF
!          ELSE
          WRITE(IO,'(3(F16.8,1x))') xx(jj),yy(jj),-9999.
          ENDIF
...
!    print *,'Jean Krug 7'
    ...
    do jj=1,nobs
       ...
       WRITE(IO,'(3(E16.8,1x))') xx(jj),yy(jj),THI(jj)
       else
...
!   FirstRound=.False.
   ...
   Return
!------------------------------------------------------------------------------
END SUBROUTINE POI_output_thickness
!------------------------------------------------------------------------------
! *****************************************************************************
