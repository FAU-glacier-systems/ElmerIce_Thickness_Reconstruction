...
!!adjoint
  ...
  !------------------------------------------------------------------------------
   ...
   IF (ComputeDJDsmbTop) THEN
     DJDsmbTopSol => VariableGet( Solver % Mesh % Variables,'DJDsmbTop' )
     IF (ASSOCIATED(DJDsmbTopSol)) THEN
       DJDsmbTop => DJDsmbTopSol % Values
       DJDsmbTopPerm => DJDsmbTopSol % Perm
     ELSE
       CALL FATAL(SolverName,'No variable <DJDsmbTop> Found')
     ENDIF
  ...
  !------------------------------------------------------------------------------
   ...
   IF (ComputeDJDUV) THEN
     DJDUVSol => VariableGet( Solver % Mesh % Variables, 'DJDUV' )
     ...
     ELSE
       CALL FATAL(SolverName,'No variable <DJDUV> Found')
     ENDIF
  ...
  !! IF solutions has been limited then derivative is 0
   ...
   CONTAINS
     ...
     !------------------------------------------------------------------------------
   END SUBROUTINE DJDp_Adjoint_ThicknessSolver
!------------------------------------------------------------------------------
