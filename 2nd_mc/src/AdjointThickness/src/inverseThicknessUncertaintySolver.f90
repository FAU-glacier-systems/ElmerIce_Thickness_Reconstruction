...
!> \ingroup Solvers
!-----------------------------------------------------------------------------
SUBROUTINE inverseThicknessUncertaintySolver( Model,Solver,dt,TransientSimulation )
  USE DefUtils
  USE Differentials
  USE MaterialModels
  IMPLICIT NONE
  ...
  DO iter=1,NonlinearIter
     ...
     DO t=1,Solver % NumberOfActiveElements
          ...
          DO i=1,n
             j = NSDOFs*FlowPerm(NodeIndexes(i))
              ...
              IF((DIM == 2) .AND. (NSDOFs == 1)) THEN 
                 Velo(1,i) = (-1.0)*FlowSolution( j ) 
                 Velo(2,i) = 0.0_dp
              !2D problem - 2D Thickness evolution (plane view pb)
              ELSE IF ((DIM == 2) .AND. (NSDOFs == 2)) THEN
                 Velo(1,i) = (-1.0)*FlowSolution( j-1 ) 
                 Velo(2,i) = (-1.0)*FlowSolution( j ) 
       ...
       ELSE
        ...
        IF (ASSOCIATED( BodyForce ) ) THEN
              ...
              LOAD(1:n) = LOAD(1:n) +   &
                      abs(GetReal( BodyForce, 'deltaSPEED_gradH', Found ))
              LOAD(1:n) = LOAD(1:n) +   &
                      abs(GetReal( BodyForce, 'deltaH_gradSPEED', Found ))
        END IF
   ...
   CONTAINS
     ...
     !------------------------------------------------------------------------------
   END SUBROUTINE inverseThicknessUncertaintySolver
!------------------------------------------------------------------------------
