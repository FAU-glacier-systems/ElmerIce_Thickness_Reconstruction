...
!> \ingroup Solvers
!-----------------------------------------------------------------------------
SUBROUTINE ThicknessSolver_mask( Model,Solver,dt,TransientSimulation )
  USE DefUtils
  USE Differentials
  USE MaterialModels
  IMPLICIT NONE
  ...
  INTEGER, POINTER ::&
       ThickPerm(:), DHDTPerm(:),FlowPerm(:), NodeIndexes(:), EdgeMap(:,:),&
       HsiaPerm(:),MaskPerm(:),SpeedPerm(:)
  ...
  REAL(KIND=dp), POINTER :: ForceVector(:), Thick(:),DHDT(:),PreH(:,:), &
       ...
       HsiaValues(:),MaskValues(:),SpeedValues(:)
  ...
  CHARACTER(LEN=MAX_NAME_LEN)  :: SolverName, VariableName, EquationName, FlowSolName, StabilizeFlag, &
       HsiaSolName,MaskSolName,SpeedSolName

  REAL(kind=dp),ALLOCATABLE :: MASK(:),SPEED(:),HSIA(:)
  REAL(kind=dp) :: speed_threshold
  TYPE(Nodes_t)   :: ElementNodes
  TYPE(Element_t),POINTER :: CurrentElement
  TYPE(Variable_t), POINTER :: FlowSol, VarThickResidual,DHDTSol,&
       HsiaSol,MaskSol,SpeedSol
  ...
  IF ( .NOT. AllocationsDone .OR. Solver % Mesh % Changed) THEN
     ...
     ALLOCATE( ElementNodes % x( NMAX ),    &
          ...
          LOAD(NMAX) , &
          HSIA(NMAX), MASK(NMAX), SPEED(NMAX), &
          Velo( 3, NMAX ), &
          MeshVelocity( 3,NMAX ), &
          Flux( 3, NMAX), &
          SourceFunc( NMAX ), &
  ...
  !------------------------------------------------------------------------------
   ...
   HsiaSolName =  GetString(SolverParams ,'Hsia Variable Name', Found)
   IF(.NOT.Found) THEN
        WRITE(Message,'(A)') &
           '<Hsia Solution Name> Not Found;'
        CALL Info(SolverName,Message,level=10)
   ELSE
        HsiaSol => VariableGet( Solver % Mesh % Variables, HsiaSolName )
        IF ( ASSOCIATED( HsiaSol ) ) THEN
             HsiaPerm     => HsiaSol % Perm
             HsiaValues => HsiaSol % Values
         ELSE
            WRITE(Message,'(A,A,A)') &
                    'No variable >',HsiaSolName,'< found'
          CALL Fatal(SolverName,Message)
         END IF
   END IF

   MaskSolName =  GetString(SolverParams ,'Mask Variable Name', Found)
   IF(.NOT.Found) THEN
        WRITE(Message,'(A)') &
           '<Mask Solution Name> Not Found;'
        CALL Info(SolverName,Message,level=10)
   ELSE
        MaskSol => VariableGet( Solver % Mesh % Variables, MaskSolName )
        IF ( ASSOCIATED( MaskSol ) ) THEN
             MaskPerm     => MaskSol % Perm
             MaskValues => MaskSol % Values
         ELSE
            WRITE(Message,'(A,A,A)') &
                    'No variable >',MaskSolName,'< found'
          CALL Fatal(SolverName,Message)
         END IF
   END IF

   SpeedSolName =  GetString(SolverParams ,'Speed Variable Name', Found)
   IF(.NOT.Found) THEN
        WRITE(Message,'(A)') &
           '<Speed Solution Name> Not Found;'
        CALL Info(SolverName,Message,level=10)
   ELSE
        SpeedSol => VariableGet( Solver % Mesh % Variables, SpeedSolName )
        IF ( ASSOCIATED( SpeedSol ) ) THEN
             SpeedPerm     => SpeedSol % Perm
             SpeedValues => SpeedSol % Values
         ELSE
            WRITE(Message,'(A,A,A)') &
                    'No variable >',SpeedSolName,'< found'
          CALL Fatal(SolverName,Message)
         END IF
   END IF
  ...
  DO iter=1,NonlinearIter
     ...
     MASK = 0.0_dp
     SPEED = 0.0_dp
     HSIA(1:n)  = HsiaValues(HsiaPerm(NodeIndexes(1:n)))
     MASK(1:n)  = MaskValues(MaskPerm(NodeIndexes(1:n)))
     SPEED(1:n) = SpeedValues(SpeedPerm(NodeIndexes(1:n)))
 
        CALL LocalMatrix_mask( STIFF, MASS, FORCE,&
             LOAD,  Velo, NSDOFs, MeshVelocity, &
             CurrentElement, n, ElementNodes, NodeIndexes, &
             TransientSimulation,&
              ALEFormulation, HSIA, MASK, SPEED, speed_threshold)
   ...
   CONTAINS
     ...
     !------------------------------------------------------------------------------
     !==============================================================================
     SUBROUTINE LocalMatrix_mask( STIFF, MASS, FORCE,&
       ...
       do p=1,nCoord
        IF (SPEED(p).LT.speed_threshold) THEN
         !print *,'SETTING HSIA',SPEED(p),speed_threshold
         STIFF = 0.0_dp
         MASS  = 0.0_dp
         FORCE = 0.0_dp
         do q=1,nCoord
         STIFF(q,q)=1.0_dp
         FORCE(q) = 1.0_dp*HSIA(q)
         enddo
        ENDIF
         !print *,'NO HSIA',SPEED(p),speed_threshold
       enddo

     !------------------------------------------------------------------------------
     END SUBROUTINE LocalMatrix_mask

     !------------------------------------------------------------------------------
   END SUBROUTINE ThicknessSolver_mask
!------------------------------------------------------------------------------
