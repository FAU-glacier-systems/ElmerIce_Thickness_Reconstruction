...
! *****************************************************************************
!> 
SUBROUTINE AdjointSolver_Flux( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
!
...
!------------------------------------------------------------------------------
  ...
  ! IF DIM = 3 and NSDOFs=1; Normal-Tangential can not be used => trick temporary set
  ! Model Dimension to 1
  IF (DIM.eq.(NSDOFs+2)) CurrentModel % Dimension = NSDOFs
  ...
  if (Firsttime) then
     ...
     ! reset Dimension to DIM
      IF (DIM.eq.(NSDOFs+2)) CurrentModel % Dimension = DIM

      Return
!------------------------------------------------------------------------------
END SUBROUTINE AdjointSolver_Flux
!------------------------------------------------------------------------------
