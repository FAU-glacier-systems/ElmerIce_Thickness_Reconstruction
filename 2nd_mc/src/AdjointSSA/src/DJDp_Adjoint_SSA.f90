...
! * 
! *****************************************************************************
SUBROUTINE DJDp_Adjoint_SSA( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
...
!!!!!!!!!!! get Solver Variables
  ...
  !! SSA Solution
     ...
     IF (AllocationsDone) DEALLOCATE(FORCE, LOAD, STIFF, NodalGravity, &
                       ...
                       NodalSliding, Nodalbetab,Nodalzsb,NodalRhob,&
                       NodalEtab,NodalZbb,&
                       ElementNodes % x, &
  ...
  !! Adjoint Solution
     VeloSolD => VariableGet( Solver % Mesh % Variables, AdjointSolName )
      ...
      IF (ComputeDJDBeta) Then
          DJDBetaSol => VariableGet( Solver % Mesh % Variables, 'DJDBeta' )
 ...
 ! set coords of highest occuring dimension to zero (to get correct path element)
     ...
     CALL LocalMatrixUVSSA (  STIFF, FORCE, Element, n, ElementNodes, NodalGravity, &
        ...
        NodalU, NodalV, NodalSliding, cn, MinSRInv , STDOFs,&
        Nodalbetab,nodalzsb,nodalzbb,nodalrhob,nodaletab)
...
!------------------------------------------------------------------------------
  SUBROUTINE LocalMatrixUVSSA(  STIFF, FORCE, Element, n, Nodes, gravity, &
           ...
           LocalV, LocalSliding, cm, MinSRInv, STDOFs , &
            nodalbetab,nodalzsb,nodalzbb,nodalrhob,nodaletab )
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
    ...
    REAL(KIND=dp) :: gradS(2),gradSb(2),Slip(2),  A(2,2),  Exx, Eyy, Exy, Ezz, Ee, MinSRInv                            
...
END SUBROUTINE DJDp_Adjoint_SSA
