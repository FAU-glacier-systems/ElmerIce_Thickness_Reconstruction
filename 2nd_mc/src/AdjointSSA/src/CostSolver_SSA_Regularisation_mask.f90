...
! *****************************************************************************
! *****************************************************************************
SUBROUTINE CostSolver_SSA_Regularisation_mask( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
!******************************************************************************
!
...
!  
  ...
  CHARACTER(LEN=MAX_NAME_LEN) :: CostSolName,VarSolName,GradSolName,varname,MaskVarName
  TYPE(Element_t),POINTER ::  Element
  TYPE(Variable_t), POINTER :: TimeVar,CostVar

  TYPE(Variable_t), POINTER :: Variable,DJDVariable,MaskVar
  REAL(KIND=dp), POINTER :: Values(:),DJDValues(:),MaskValues(:)
  INTEGER, POINTER :: Perm(:),DJDPerm(:),MaskPerm(:)
  ...
  real(kind=dp) :: Cost,Cost_S,Lambda,Lambda0
  real(kind=dp) :: u,v,w,s,coeff_reg,SqrtElementMetric,x,maskit,topf
  ...
  REAL(KIND=dp) :: IPerr,IPvar,NodeMaskValues(3)

  LOGICAL :: Apriori,Reset,UseMask

  CHARACTER*10 :: date,temps

  save Firsttime,Parallel 
  save SolverName,CostSolName,VarSolName,CostFile
  save ElementNodes
...
!! Mask for various lambda
   MaskVarName = GetString( SolverParams,'Weighting Mask Name',UseMask)
!    print *,'JEAN KRUG 00',MaskVarName 
...
!    print *,'JEAN KRUG 31'
      NodeMaskValues(1:n)=MaskValues(MaskPerm(NodeIndexes(1:n)))

!      NodeMaskValues(1)=1.0
!       NodeMaskValues(2)=1.0
!      NodeMaskValues(3)=1.0
      maskit = 0.0_dp
!    print *,'JEAN KRUG 4'
!      print *,'MASKIT BEFORE',maskit
      IF (UseMask)THEN
        do i = 1,3
        maskit = maskit+NodeMaskValues(i)
        enddo
      ENDIF
!      print *,'MASKIT AFTER',maskit
!       maskit = maskit
!      !maskit = SUM(MaskNodeValues(1:n))/n
!
!        !topf=Lambda
        Lambda0=Lambda
        if(maskit.gt.0.5)then
!        print *,'maskit - Lambda0',maskit,Lambda0
        Lambda0=Lambda*10.0_dp
!        topf  = 1.0e-1
!        endif
       ELSE
!       topf=1.0
          ...
          IF (Apriori) then
             ...
             !Now compute the derivative
               NodalRegb(1:n)=NodalRegb(1:n)+&
                    s*Lambda0*IPerr*Basis(1:n)/(IPVar**2.0)
          Else
!             NodeMaskValues(1)=1.0
!             NodeMaskValues(2)=1.0
!             NodeMaskValues(3)=-1.0
!             maskit=(NodeMaskValues(1)+NodeMaskValues(2)+NodeMaskValues(3))/n
!             !maskit=3.
!             topf=1.0
!             if(maskit.lt.3)then
!             topf=1.0e-1
!             endif
             ...
             !Now compute the derivative
               NodalRegb(1:n)=NodalRegb(1:n)+&
                    s*Lambda0*SUM(NodeValues(1:n)* dBasisdx(1:n,1))*dBasisdx(1:n,1)
               IF (DIM.eq.2) then
                  NodalRegb(1:n)=NodalRegb(1:n)+&
                          s*Lambda0*SUM(NodeValues(1:n)*dBasisdx(1:n,2))*dBasisdx(1:n,2)
               End if
          Endif

          Cost=Cost+0.5*Lambda0*coeff_reg*s
...
!   Cost=0.5*Lambda*Cost
   ...
   Return
!------------------------------------------------------------------------------
END SUBROUTINE CostSolver_SSA_Regularisation_mask
!------------------------------------------------------------------------------
! *****************************************************************************
